...
if (fs.existsSync(envPath)) {
  ...
  lines.forEach((line) => {
    if (line.startsWith("ROUTESCAN_API_KEY=")) {
      routescanApiKey = line.split("=")[1].trim();
    }
  });
}

async function verifyOnRoutescan(
  apiUrl,
  apiKey,
  contractAddress,
  contractName,
  ...
  console.log(`API Key: ${apiKey ? "✅ Present" : "⚠️  Using default"}\n`);
  ...
  try {
    ...
    params.append("apikey", apiKey || "cgtextrk");
...
async function main() {
  console.log("\n╔════════════════════════════════════════════════════════════╗");
  console.log("║  ROUTESCAN API - CONTRACT VERIFICATION                     ║");
  console.log("╚════════════════════════════════════════════════════════════╝");

  console.log(`\n📋 Configuration:\n`);
  console.log(`Base OFT:        ${baseOftAddr}`);
  console.log(`Arbitrum OFT:    ${arbOftAddr}`);
  console.log(`Routescan Key:   ${routescanApiKey ? "✅ Set" : "⚠️  Using default"}\n`);

  console.log(`═`.repeat(70));
  console.log(`Using Routescan (unified API for Base & Arbitrum)`);
  console.log(`═`.repeat(70));

  // Verify Base OFT
  const baseResult = await verifyOnRoutescan(
  ...
  // Verify Arbitrum OFT
  const arbResult = await verifyOnRoutescan(
  ...
  if (baseResult.success) {
    if (baseResult.alreadyVerified) {
      console.log(`  ✅ Already verified on Routescan`);
    } else {
      console.log(`  ✅ Verification submitted successfully`);
