...
export function TxStatusBanner({ status }: TxStatusBannerProps) {
  ...
  return (
    <motion.div
      key={`${status.source}-${status.stage}-${status.txHash || ''}`}
    ...
    >
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
        ...
        <button type="button" className="rounded-full border border-slate-900/10 bg-white/90 px-2.5 py-1 text-xs font-semibold">
